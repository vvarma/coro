...
TEST_CASE("enumerate sequence of 1 value") {
  coro::sync_wait([]() -> coro::task<> {
    bool startedExecution = false;
    auto makeGenerator = [&]() -> coro::async_generator<std::uint32_t> {
      startedExecution = true;
      co_yield 1;
    };
...
class set_to_true_on_destruction {
 public:
  ...
  set_to_true_on_destruction &operator=(const set_to_true_on_destruction &) =
      delete;
...
TEST_CASE(
    "destructors of values in scope are called when async_generator "
    "destructed early") {
  coro::sync_wait([]() -> coro::task<> {
    bool aDestructed = false;
    bool bDestructed = false;

    auto makeGenerator = [&](set_to_true_on_destruction a)
        -> coro::async_generator<std::uint32_t> {
...
TEST_CASE(
    "async producer with async consumer" *
    doctest::description{
...
TEST_CASE(
  ...
  class TestException {};
  auto gen = [](bool shouldThrow) -> coro::async_generator<std::uint32_t> {
    if (shouldThrow) {
      throw TestException();
    }
    co_yield 1;
...
TEST_CASE(
    "large number of synchronous completions doesn't result in "
    "stack-overflow") {
  auto makeSequence = [](coro::single_consumer_event &event)
      -> coro::async_generator<std::uint32_t> {
  ...
  auto consumer =
      [](coro::async_generator<std::uint32_t> sequence) -> coro::task<> {
    std::uint32_t expected = 0;
    for (auto iter = co_await sequence.begin(); iter != sequence.end();
         co_await ++iter) {
      std::uint32_t i = *iter;
      CHECK(i == expected++);
    }
  ...
  auto unblocker = [](coro::single_consumer_event &event) -> coro::task<> {
    // Should have processed the first 500'000 elements synchronously with
    // consumer driving iteraction before producer suspends and thus consumer
    // suspends. Then we resume producer in call to set() below and it continues
    // processing remaining 500'000 elements, this time with producer driving
    // the interaction.
  ...
  coro::sync_wait(
      coro::when_all_ready(consumer(makeSequence(event)), unblocker(event)));
}
