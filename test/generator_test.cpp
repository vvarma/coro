...
TEST_CASE("generator of arithmetic type returns by copy") {
  ...
  // static_assert(std::is_same<decltype(*iter), float>::value, "operator*
  // should return float by value");
  CHECK(*iter == 1.0f);
  ++iter;
  CHECK(*iter == 2.0f);
  ++iter;
...
TEST_CASE("value-category of fmap() matches reference type") {
  ...
  consume([]() -> generator<const int> { co_yield 123; }() |
                      fmap(checkIsConstLvalue));
  consume([]() -> generator<int &> { co_yield 123; }() | fmap(checkIsLvalue));
  consume([]() -> generator<const int &> { co_yield 123; }() |
                      fmap(checkIsConstLvalue));
  consume([]() -> generator<int &&> { co_yield 123; }() | fmap(checkIsRvalue));
  consume([]() -> generator<const int &&> { co_yield 123; }() |
                      fmap(checkIsConstRvalue));
}
...
TEST_CASE("generator throwing before yielding first element rethrows out of "
          "begin()") {
  class X {};
...
TEST_CASE("fmap operator") {
  coro::generator<int> gen =
      range(0, 5) | coro::fmap([](int x) { return x * 3; });
...
// HACK: Disable this test as it's causing heap corruption errors under MSVC
// 2017 Update 5 x86 debug builds. Still needs investigation of root cause.
TEST_CASE("low_pass" * doctest::skip{true}) {
  ...
  {
    auto gen =
        low_pass<4>(std::forward_list<int>{10, 13, 10, 15, 18, 9, 11, 15});
    auto it = gen.begin();
    CHECK(*it == 10.0);
    CHECK(*++it == 11.5);
    CHECK(*++it == 11.0);
