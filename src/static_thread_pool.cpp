...
namespace coro {
thread_local static_thread_pool::thread_state*
    static_thread_pool::s_currentState = nullptr;
thread_local static_thread_pool* static_thread_pool::s_currentThreadPool =
    nullptr;
...
class static_thread_pool::thread_state {
 public:
  ...
  bool try_local_enqueue(schedule_operation*& operation) noexcept {
    ...
    // We can now re-read tail, guaranteed that we are not seeing a stale
    // version.
...
void static_thread_pool::remote_enqueue(
    schedule_operation* operation) noexcept {
...
bool static_thread_pool::approx_has_any_queued_work_for(
    std::uint32_t threadIndex) const noexcept {
  // Cheap, approximate, read-only implementation that checks whether any work
  // has been queued in the system somewhere. We try to avoid writes here so
  // that we don't bounce cache-lines around between threads/cores unnecessarily
  // when multiple threads are all spinning waiting for work.
...
void static_thread_pool::try_clear_intent_to_sleep(
    std::uint32_t threadIndex) noexcept {
  // First try to claim that we are waking up one of the threads.
  std::uint32_t oldSleepingCount =
      m_sleepingThreadCount.load(std::memory_order_relaxed);
  do {
    if (oldSleepingCount == 0) {
      ...
      oldSleepingCount, oldSleepingCount - 1, std::memory_order_acquire,
      std::memory_order_relaxed));
...
static_thread_pool::schedule_operation*
static_thread_pool::try_steal_from_other_thread(
...
void static_thread_pool::wake_one_thread() noexcept {
  ...
  std::uint32_t oldSleepingCount =
      m_sleepingThreadCount.load(std::memory_order_seq_cst);
  do {
    if (oldSleepingCount == 0) {
      // No sleeping threads.
      // Someone must have woken us up.
